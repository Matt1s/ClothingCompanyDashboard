defmodule ClothingCompanyDashboard.Inventory do
  ...
  def list_products_by_attributes(categories \\ nil, price_min \\ nil, price_max \\ nil, name \\ nil, in_stock \\ nil) do


    # First, check for empty values and set them to nil
    categories = if categories == "" do nil else categories end
    ...
    Product
    |> maybe_filter_by_categories(categories)
  ...
  defp maybe_filter_by_categories(query, categories), do: (from p in query, where: p.category in ^categories)
